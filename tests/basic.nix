...
in
{
  ...
  testScript = { nodes, ... }:
    let
      ...
      allFiles = lib.flatten (lib.mapAttrsToList (_: preservationLib.getAllFiles) nodes.machine.preservation.preserveAt);
      allDirs = lib.flatten (lib.mapAttrsToList (_: preservationLib.getAllDirectories) nodes.machine.preservation.preserveAt);
