...
let
  ...
  directoryPath = { name, defaultOwner, ... }: {
    options = {
      ...
      mountOptions = lib.mkOption {
        type = with lib.types; listOf (
          coercedTo str (n: { name = n; }) mountOption
        );
        default = [
          "bind"
      ...
      how = lib.mkOption {
        type = lib.types.enum [ "bindmount" "symlink" ];
        default = "bindmount";
        description = ''
  ...
  filePath =
    { name, defaultOwner, ... }: {
      options = {
        ...
        mountOptions = lib.mkOption {
          type = with lib.types; listOf (
            coercedTo str (o: { name = o; }) mountOption
          );
          default = [
            "bind"
          ];
  ...
  userModule = { name, ... }@attrs: {
    options = {
      ...
      directories = lib.mkOption {
        ...
        default = [ ];
        apply = map (d: d // {
          directory = "${attrs.config.home}/${d.directory}";
        });
        description = ''
  ...
  config = lib.mkIf cfg.enable {
    ...
    boot.initrd.systemd = {
      ...
      tmpfiles.settings.preservation =
        lib.mkMerge (lib.flatten (lib.mapAttrsToList mkInitrdTmpfilesRules cfg.preserveAt));
      mounts =
        lib.flatten (lib.mapAttrsToList mkInitrdMountUnits cfg.preserveAt);
    };
    ...
    systemd = {
      ...
      tmpfiles.settings.preservation =
        lib.mkMerge (lib.flatten (lib.mapAttrsToList mkRegularTmpfilesRules cfg.preserveAt));
      mounts =
        lib.flatten (lib.mapAttrsToList mkRegularMountUnits cfg.preserveAt);
    };
